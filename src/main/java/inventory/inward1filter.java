...
@RestController
@RequestMapping("/api/inward1filter")
public class inward1filter {
    ...
    public List<Map<String, Object>> getInventory(
            ...
            @RequestParam(value = "specific_date", required = false) String specificDate
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT po_no, stock_name, store_name, date, quantity, amount " +
            "FROM inlet_stock WHERE 1=1"
        );
        ...
        if (storeName != null && !storeName.isEmpty()) {
            sql.append(" AND store_name = ?");
            params.add(storeName);
        }

        // ✅ Specific Date Overrides Date Range
        if (specificDate != null && !specificDate.isEmpty()) {
            sql.append(" AND date = ?");
            params.add(specificDate);
        } else {
    ...
    // Helper Methods
    // --------------------------------
    private List<Integer> getUniqueIntValues(String column)
            throws SQLException, ClassNotFoundException {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             ...
             ResultSet rs = stmt.executeQuery(
                     "SELECT DISTINCT " + column + " FROM inlet_stock ORDER BY " + column)) {
    ...
    private List<String> getUniqueStringValues(String column)
            throws SQLException, ClassNotFoundException {
